...
echo "============================================"
echo "FlyMine Build Container"
echo "============================================"

# ============================================
# Compile bio-sources and flymine (first run only)
# Source is baked into the image by Dockerfile (COPY source/flymine* ...)
# and the bintray-strip + gradle heap append already happened at image
# build time, so this just needs to install.
# ============================================
compile_if_needed() {
...
# ============================================
resolve_release() {
    if [ -n "${FLYMINE_RELEASE}" ]; then
        echo "FlyMine Release: ${FLYMINE_RELEASE} (from env)"
        return
    fi

    export FLYMINE_RELEASE
    FLYMINE_RELEASE="$(date +%Y%m%d)"
    echo "FLYMINE_RELEASE not set, defaulting to ${FLYMINE_RELEASE}"
}
...
resolve_rc_number() {
    ...
    local ver_sanitized
    ver_sanitized=$(echo "${FLYMINE_RELEASE}" | tr '.' '_')
    local pattern="flymine_${ver_sanitized}_rc"
...
construct_db_names() {
    ...
    RELEASE_SANITIZED=$(echo "${FLYMINE_RELEASE}" | tr '.' '_')

    if [ "${BUILD_TYPE}" = "production" ]; then
        export RDS_DB_NAME="flymine_${RELEASE_SANITIZED}"
    else
        RC="${RC_NUMBER:-1}"
        export RDS_DB_NAME="flymine_${RELEASE_SANITIZED}_rc${RC}"
    fi

    if [ "${BUILD_TYPE}" = "production" ]; then
        export RDS_PROFILE_DB_NAME="flymine_userprofile"
    elif [ "${COPY_PROFILE_DB}" = "true" ]; then
        export RDS_PROFILE_DB_NAME="flymine_userprofile_rc${RC_NUMBER:-1}"
    else
        export RDS_PROFILE_DB_NAME="flymine_userprofile_test"
    fi

    export RDS_ITEMS_DB_NAME="flymine_items"
...
configure_properties() {
    ...
    cp /root/.intermine/flymine.properties \
       /root/flymine/flymine.properties 2>/dev/null || true
...
setup_databases() {
    ...
    if ! db_exists "${RDS_PROFILE_DB_NAME}"; then
        if [ "${BUILD_TYPE}" != "production" ] && db_exists "flymine_userprofile"; then
            echo "  Copying profile DB: flymine_userprofile -> ${RDS_PROFILE_DB_NAME}"
            ${PSQL} -d postgres -c \
                "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='flymine_userprofile' AND pid <> pg_backend_pid();" \
                2>/dev/null || true
            ${PSQL} -d postgres -c \
                "CREATE DATABASE \"${RDS_PROFILE_DB_NAME}\" TEMPLATE \"flymine_userprofile\";"
